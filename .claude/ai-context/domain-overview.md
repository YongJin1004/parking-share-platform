...
### 등록 플로우 (Host)
1. **주소 검색**: Kakao 우편번호 서비스 WebView로 주소 선택
2. **운영 날짜/시간 선택**: 달력에서 날짜 멀티셀렉, 날짜별 시작/종료 시간(정시 단위) + 요금 입력
3. **허용 차종 선택**: sedan/suv/van/truck/motorcycle 멀티셀렉 (선택 안 하면 모두 허용)
4. **사진 업로드**: 기기 사진 보관함에서 선택 → 등록 후 서버에 업로드
5. **상세 설명**: 선택 입력

### 운영 시간 규칙
- 시간 단위만 선택 가능 (분은 항상 00분 고정)
- 날짜별로 다른 시간대/요금 설정 가능
- hourly_rate는 스케줄 최솟값으로 자동 설정
